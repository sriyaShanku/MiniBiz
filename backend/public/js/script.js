document.addEventListener('DOMContentLoaded', () => {
    ...
    */
    ...
    // Ensure these are defined globally or accessible where needed
    const contactModal = document.getElementById('contact-seller-modal');
    const modalContactSellerNamePlaceholder = document.getElementById('modal-contact-seller-name-placeholder');
    const modalContactProductNamePlaceholder = document.getElementById('modal-contact-product-name-placeholder');   
    const modalContactSellerIdInput = document.getElementById('modal-contact-seller-id-input');
    const modalContactProductIdInput = document.getElementById('modal-contact-product-id-input');
    const modalContactForm = document.getElementById('modal-contact-form');
    const modalContactFormStatus = document.getElementById('modal-contact-form-status');
    const modalContactMessageTextarea = document.getElementById('modal-contact-message');
    const modalContactEmailInput = document.getElementById('modal-contact-email'); // For guest email

    function openContactModal(buttonElement) {
        if (!contactModal || !modalContactForm) {
            console.error("Contact modal elements not found!");
            return;
        }

        const sellerId = buttonElement.dataset.sellerId;
        const sellerName = buttonElement.dataset.sellerName || 'Seller'; // Fallback
        const productId = buttonElement.dataset.productId; // Might be undefined for general seller contact
        const productName = buttonElement.dataset.productName; // Might be undefined

        if (modalContactSellerNamePlaceholder) {
            modalContactSellerNamePlaceholder.textContent = sellerName;
        }
        if (modalContactSellerIdInput) {
            modalContactSellerIdInput.value = sellerId || '';
        }

        if (productId && productName && modalContactProductNamePlaceholder) {
            modalContactProductNamePlaceholder.textContent = productName;
            modalContactProductNamePlaceholder.style.display = 'inline'; // Show it
            // Also update the preceding text if needed
            const precedingText = modalContactProductNamePlaceholder.previousSibling;
            if (precedingText && precedingText.nodeType === Node.TEXT_NODE) {
                precedingText.textContent = 'Send your inquiry about ';
            }

        } else if (modalContactProductNamePlaceholder) {
            modalContactProductNamePlaceholder.style.display = 'none'; // Hide it
            const precedingText = modalContactProductNamePlaceholder.previousSibling;
            if (precedingText && precedingText.nodeType === Node.TEXT_NODE) {
                precedingText.textContent = 'Send your inquiry to this seller.';
            }
        }

        if (modalContactProductIdInput) {
            modalContactProductIdInput.value = productId || ''; // Empty if no product ID
        }

        if (modalContactFormStatus) {
            modalContactFormStatus.style.display = 'none';
            modalContactFormStatus.textContent = '';
        }
        if (modalContactMessageTextarea) {
            modalContactMessageTextarea.value = ''; // Clear previous message
        }
        // If guest email field exists and is visible, clear it
        if (modalContactEmailInput && modalContactEmailInput.offsetParent !== null) { // Check if visible
            modalContactEmailInput.value = '';
        }


        contactModal.style.display = 'block';
        document.body.style.overflow = 'hidden';
        if(modalContactMessageTextarea) modalContactMessageTextarea.focus();
    }
    ...
    // Event listener for the modal form submission (AJAX)
    if (modalContactForm) {
        modalContactForm.addEventListener('submit', async function(event) {
            event.preventDefault();
            if (modalContactFormStatus) {
                modalContactFormStatus.style.display = 'block';
                modalContactFormStatus.textContent = 'Sending...';
                modalContactFormStatus.style.color = 'var(--text-muted)';
            }
            ...
            if (!data.seller_id) {
                if (modalContactFormStatus) {
                    modalContactFormStatus.textContent = 'Error: Seller ID is missing.';
                    modalContactFormStatus.style.color = 'red';
                }
                console.error('Seller ID missing in contact form data.');
                return;
            }

            try {
                const response = await fetch('/messages/send', { // Your backend API endpoint
                    method: 'POST',
                    headers: {
                        'Content-Type': 'application/json',
                ...
                if (modalContactFormStatus) {
                    modalContactFormStatus.textContent = result.message;
                    if (result.success) {
                        modalContactFormStatus.style.color = 'green';
                        modalContactForm.reset(); // Clear the form
                        setTimeout(closeContactModal, 2500); // Close modal after a delay
                    } else {
                        modalContactFormStatus.style.color = 'red';
                    }
                }
                ...
                if (modalContactFormStatus) {
                    modalContactFormStatus.textContent = 'An error occurred. Please try again.';
                    modalContactFormStatus.style.color = 'red';
                }
            }
       });
    ...
    // Close modal if clicking on the overlay or close button (if using event delegation)
    if (contactModal) {
        contactModal.addEventListener('click', function(event) {
            if (event.target === contactModal || event.target.classList.contains('modal-close-btn')) {
                closeContactModal();
            }
       });
    }
